...
\begin{figure}[H]
	...
	\includegraphics[width=.85\linewidth]{General/Images/sd_flashing_overview.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.9\linewidth]{General/Images/jetson_top_layout.jpg}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.9\linewidth]{General/Images/wiring_overview.jpg}
...
\begin{itemize}
	\item \textbf{No display:} Check HDMI input source; try a different cable; test at \SI{1920x1080} first.
...
% -------------------------------------------------------
% Illustrative Figures (you provide images in General/Images/)
% -------------------------------------------------------

\begin{figure}[H]
	\centering
	\includegraphics[width=.9\linewidth]{General/Images/camera_mount_example.jpg}
	\caption{Example camera mounting for a stable field of view (optional tripod or bracket).}
	\label{fig:cam-mount}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=.9\linewidth]{General/Images/first_boot_wizard.jpg}
	\caption{First boot configuration wizard: locale, user account, and display settings.}
	\label{fig:first-boot}
\end{figure}

% -------------------------------------------------------
% Optional: Compact TikZ flow just for setup logic (no ethernet)
% -------------------------------------------------------
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[
		every node/.style={font=\scriptsize, align=center},
		step/.style={draw, rounded corners=2pt, fill=blue!10, minimum width=2.8cm, minimum height=0.9cm},
		ok/.style={draw, rounded corners=2pt, fill=green!10, minimum width=2.8cm, minimum height=0.9cm},
		arr/.style={-{Latex[length=2mm]}, thick}
		]
		\node[step] (flash) {Flash microSD\\(JetPack 5.1.2)};
		\node[step, right=1.8cm of flash] (wire) {Connect HDMI,\\USB Camera,\\KB/Mouse};
		\node[step, right=1.8cm of wire] (power) {Apply \SI{5}{V}/\SI{4}{A}\\Power};
		\node[step, right=1.8cm of power] (wizard) {Run First-Boot\\Wizard};
		\node[ok,   right=1.8cm of wizard] (test) {Test Camera\\Preview};
		
		\draw[arr] (flash) -- (wire);
		\draw[arr] (wire) -- (power);
		\draw[arr] (power) -- (wizard);
		\draw[arr] (wizard) -- (test);
	\end{tikzpicture}
	\caption{Setup flow: from SD flashing to live camera preview.}
