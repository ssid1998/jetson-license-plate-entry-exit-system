...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!60!black,title={\faCar\hspace{0.5em}Manaul - Licence plate detection using Jetson nano }]
	This project implements an \textbf{Automatic License Plate Detection and Recognition System} using the \textbf{NVIDIA Jetson Nano} edge device.
	
	The system captures live video from a USB camera, detects vehicle license plates using the \textbf{YOLOv8} model, and extracts the plate number using \textbf{EasyOCR}.  
	Recognized plate data are stored locally in an \textbf{SQLite database} with timestamps and confidence values.  
	It is designed for real-time \textbf{vehicle logging, parking automation}, and \textbf{access control}.
\end{tcolorbox}
...
% --------------------------------------------------
\begin{tcolorbox}[colback=red!5!white,colframe=red!60!black,title={\faTools\hspace{0.5em}Troubleshooting}]
	\begin{itemize}
...
\begin{center}
	\begin{tabular}{ll}
		\textbf{Device:} & Seeed Studio reComputer J1010 (Jetson Nano 4 GB)\\
		\textbf{OS:} & JetPack 5.1.2 (Ubuntu 20.04)\\
		\textbf{Processor:} & Quad-core ARM Cortex-A57 CPU\\
		\textbf{GPU:} & 128-core Maxwell\\
		\textbf{Memory:} & 4 GB LPDDR4\\
		\textbf{Storage:} & 64 GB microSD card\\
		\textbf{Camera:} & Microsoft LifeCam Show (USB 2.0)\\
		\textbf{Power:} & 5 V / 4 A USB-C\\
		\textbf{Software:} & YOLOv8 + EasyOCR + SQLite + OpenCV + Python 3.8\\
	\end{tabular}
\end{center}