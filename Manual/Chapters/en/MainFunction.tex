...
\begin{figure}[h!]
	\centering
%	\includegraphics[width=0.8\textwidth]{images/billing_flow.png}
	\caption{Automatic Billing Workflow for Parking Duration}
	\label{fig:billing_flow}
\end{figure}
...
The billing process automatically determines whether a vehicle should be charged based on its total parking duration. If the total duration is less than 15 inutes it is not charged and if it is greater than 15 minutes 2 Euros is charged .The workflow is divided into four main scenarios as described below.

\subsection{Vehicle Entry}
\begin{itemize}
	\item When a car enters the premises, the main unit automatically detects and records the license plate number.
	\item The entry time is stored in the database with a timestamp.
	\item Then the Driver is shown with the Welcome message(Eg. Willkommen - EMD AB 1234 - Timestamp ) with the Number plate  with the Time stamp
	\item No payment is initiated at this stage.
%	\begin{center}
%		\includegraphics[width=0.5\textwidth]{General/entry.png}
%	\end{center}
\end{itemize}

\subsection{Vehicle Exit}
\begin{itemize}
	\item On exit, the main unit again detects the license plate and retrieves the corresponding entry record.
	\item The current time is compared with the stored entry timestamp.
	\item The total parking duration is computed in minutes.
%	\begin{center}
%		\includegraphics[width=0.8\textwidth]{General/end.png}
%	\end{center}
\end{itemize}

\subsubsection{Scenario 1: Duration Less Than 15 Minutes}
\begin{itemize}
	\item If the total parking time is \textbf{less than 15 minutes}, the main unit classifies the session as \textbf{free of cost}.
	\item The details of the parking session are Displayed
	\item The display automatically displays to the \textbf{“Danke” (Thank You)} page (Eg. Danke - EMD AB 1234 - Total time ).
	\item The barrier opens and the vehicle exits without any payment.
%	\begin{center}
%		\includegraphics[width=0.8\textwidth]{general/exit<15.png}
%	\end{center}
\end{itemize}

\subsubsection{Scenario 2: Duration Greater Than 15 Minutes}
\begin{itemize}
	\item If the parking duration \textbf{exceeds 15 minutes}, the main unit calculates the billing amount is at  flat 2 Euros
	\item The total payable amount is shown on the display screen.
	\item Once the transaction is successful , Car is allowed to exit.
%	\begin{center}
%		\includegraphics[width=0.8\textwidth]{General/exit>15.png}
%	\end{center}
\end{itemize}
...
%
%\vspace{0.4em}
%All processing happens inside the compact system.  
...
\begin{itemize}
	...
	\item The main unit  is capable of detecting German license plates and validating them. In the event , where the non German plate is recognized the main unit displays an 'Invalid Licence plate ' .
\end{itemize}
