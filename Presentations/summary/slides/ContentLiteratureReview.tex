% !TeX encoding = utf8
% !TeX root = Rename
%
%%%%%%

\Mysection{Introduction}

\STANDARD{Introduction I}
{
	\begin{itemize}
		\item The project focuses on designing an automatic car license plate detection system using the Nvidia Jetson Nano.
		\item  License plates are detected using ML models (CNN) deployed on embedded devices. \cite{Tan+22} \cite{jocher2023ultralytics,Cor21}.		
		\item Application: Places with high vehicle throughput such as airport departures, where manual verification is inefficient and error-prone. 
		 
	\end{itemize}
	
}

\Mysection{Description of Application}
	
\STANDARD{Description of Application I}
{
	\begin{itemize}
		\item \textbf{Target deployments:} Airport curbside (drop-off time control) and parking facilities (paid stay management).
		\item \textbf{Core idea:} detect the license plate at \textbf{entry} and \textbf{exit} using a camera + Jetson Nano, and use timestamps to compute dwell time.
		\item \textbf{Edge-first operation:} Recognition and decision making run locally enabling low latency. Reduced dependence on centralized servers.
		\item \textbf{Operator-specific configuration (before delivery):} the airport/parking authority provides billing requirements (tariff, grace period, rounding rules); we configure the system accordingly and deploy it.
		\item \textbf{Output:} detects licence plate, entry/exit events, and a fee/permission decision shown on a display.
	\end{itemize}
}

\STANDARD{Description of Application II}
{
%	\begin{itemize}
%		\item \textbf{Entry flow:} camera captures vehicle $\rightarrow$ YOLO detects license plate $\rightarrow$ OCR reads text $\rightarrow$ store \texttt{(plate\_id, entry\_timestamp)} in database.
%		\item \textbf{Exit flow:} detect plate again $\rightarrow$ retrieve \texttt{entry\_timestamp} $\rightarrow$ compute dwell time \texttt{(exit - entry)}.
%		\item \textbf{Billing logic (configured per operator):} apply provided parameters (grace minutes, \euro{}/hour tariff, rounding) $\rightarrow$ compute amount due.
%		\item \textbf{Decision \& display:} show “free exit” or “payment required” with the calculated amount due.
%		\item \textbf{Logging:} write \texttt{exit\_timestamp} and fee outcome for auditing and analytics.
%	\end{itemize}

% ------------------------------------------------------------
% TikZ figure for "Description of Application II" (Workflow)
% Place this inside your \STANDARD{Description of Application II}{ ... }
% (e.g., below the itemize, or replace the itemize if you prefer)
% ------------------------------------------------------------

\begin{figure}[h]
	\centering
	\begin{tikzpicture}[
		scale=0.43,
		transform shape,
		node distance=10mm and 12mm,
		font=\small,
		box/.style={draw, rounded corners, align=center, inner sep=5pt, minimum width=3.2cm, minimum height=0.95cm},
		bigbox/.style={draw, rounded corners, align=center, inner sep=6pt, minimum width=3.6cm, minimum height=1.1cm},
		arrow/.style={->, line width=0.6pt}
		]
		
		% --- ENTRY LANE FLOW (top row)
		\node[box] (cam_in) {Camera\\(Entry)};
		\node[box, right=of cam_in] (yolo_in) {YOLOv8\\Plate Detection};
		\node[box, right=of yolo_in] (ocr_in) {OCR\\Text Extraction };
		\node[bigbox, right=of ocr_in] (db_write) {Local Database\\\texttt{(number\_plate, entry\_timestamp)}};
		
		\draw[arrow] (cam_in) -- (yolo_in);
		\draw[arrow] (yolo_in) -- (ocr_in);
		\draw[arrow] (ocr_in) -- (db_write);
		
		% --- EXIT LANE FLOW (bottom row)
		\node[box, below=18mm of cam_in] (cam_out) {Camera\\(Exit)};
		\node[box, right=of cam_out] (yolo_out) {YOLOv8\\Plate Detection\\ + \\OCR Text Extraction };
		\node[box, right=of yolo_out] (db_read) {Retrieve\\\texttt{entry\_timestamp}};
		
		\draw[arrow] (cam_out) -- (yolo_out);
		\draw[arrow] (yolo_out) -- (db_read);
		
		% Link to DB
		\draw[arrow] (db_read.north) -- ++(0,6mm) -| (db_write.south);
		
		% --- DWELL + BILLING + DECISION (center/right)
		\node[box, right=of db_read] (dwell) {Compute\\Dwell Time\\\texttt{(exit-entry)}};
		\node[bigbox, right=of dwell] (billing) {Billing Logic\\(grace, \euro{}/hour, rounding)};
		\node[box, right=of billing] (display) {Decision \& Display\\Free exit / Pay};
		
		\draw[arrow] (db_read) -- (dwell);
		\draw[arrow] (dwell) -- (billing);
		\draw[arrow] (billing) -- (display);
		
		% --- LOGGING (bottom right)
		\node[box, below=18mm of display] (log) {Logging\\\texttt{exit\_timestamp, fee       .}};
		\draw[arrow] (display.south) -- (log.north);
%		\draw[arrow] (billing.south) -- (log.north);
		
		% --- Optional labels for clarity
		\node[font=\small\bfseries, above=2mm of cam_in] {Entry Flow};
		\node[font=\small\bfseries, above=2mm of cam_out] {Exit Flow};
		
	\end{tikzpicture}
	\caption{End-to-end workflow for entry/exit recognition, dwell-time computation, billing, and logging.}
	\label{fig:app2_workflow}
\end{figure}

}

\STANDARD{Description of Application III}
{
	\begin{figure}
		\centering
		\includegraphics[width=1.0\textwidth]{img/lp_setup.png}
		\caption{ License Plate Detection System.}
	\end{figure}
}

\Mysection{Challenges and Solutions}

\STANDARD{Challenge 1: Storage Constraints}
{
	\renewcommand{\arraystretch}{1.5}
	\small
	
	\noindent
	\begin{tabular}{|p{0.48\linewidth}|p{0.48\linewidth}|}
		\hline
		\cellcolor{red!40!black}\textbf{\color{white} The Problem} & 
		\cellcolor{blue!40!black}\textbf{\color{white} The Solution} \\ 
		\hline
		\cellcolor{gray!10}
		\textbf{Hardware Limitation (Production Module)} \newline
		$\bullet$ \textbf{Disabled SD card Slot:} Our unit is the \textit{Jetson Nano Production Module} (P3448), which disables the SD card slot by design. \newline
		$\bullet$ \textbf{Space Lock:} The fixed 16GB internal storage (eMMC) is too small for our AI environment (CUDA + OS).
		& 
		\cellcolor{gray!10}
		\textbf{External Boot Architecture} \newline
		$\bullet$ \textbf{SSD Integration:} We engineered the system to bypass the internal memory and boot directly from an external 500GB SSD via USB 3.0. \newline
		$\bullet$ \textbf{Result:} Solved the space issue and increased data read speeds. \\
		\hline
	\end{tabular}
}
% ==============================================================================
% SLIDE 1: MEMORY CHALLENGE
% ==============================================================================
\STANDARD{Challenge 2: System Memory Exhaustion}
{
	\renewcommand{\arraystretch}{1.5} 
	\small 
	
	\noindent
	\begin{tabular}{|p{0.48\linewidth}|p{0.48\linewidth}|}
		\hline
		\cellcolor{red!40!black}\textbf{\color{white} The Problem} & 
		\cellcolor{blue!40!black}\textbf{\color{white} The Engineered Solution} \\ 
		\hline
		\cellcolor{gray!10}
		\textbf{Conflict: Unified Memory Bottleneck} \newline
		The Jetson Nano shares a single 4GB RAM pool between the CPU and GPU.
		\newline \newline
		$\bullet$ \textbf{OS Overhead:} Ubuntu 18.04 consumes $\approx$ 1.2 GB. \newline
		$\bullet$ \textbf{AI Load:} YOLO + EasyOCR required $\approx$ 3.3 GB during initialization.
		\newline \newline
		\textit{Impact:} Demand (4.5 GB) exceeded physical limits. The Linux kernel invoked the OOM Killer and terminated the process instantly.
		& 
		\cellcolor{gray!10}
		\textbf{Solution: Hybrid Memory Architecture}:
		\newline \newline
		$\bullet$ \textbf{Virtual Expansion:} Configured a \textbf{6GB Swap Partition} on an external SSD to offload inactive OS pages, keeping physical RAM free for inference. \newline
		\newline			
		$\bullet$ \textbf{Compute Isolation:} Forced OCR to run on the \textbf{CPU} (\texttt{gpu=False}). This reserves the limited GPU VRAM entirely for YOLO detection. \\
		\hline
	\end{tabular}
}

% ==============================================================================
% SLIDE 2: SOFTWARE CHALLENGE
% ==============================================================================
\STANDARD{Challenge 3: Legacy Software Ecosystem}
{
	\renewcommand{\arraystretch}{1.5}
	\small
	
	\noindent
	\begin{tabular}{|p{0.48\linewidth}|p{0.48\linewidth}|}
		\hline
		\cellcolor{red!40!black}\textbf{\color{white} The Problem} & 
		\cellcolor{blue!40!black}\textbf{\color{white} The Engineered Solution} \\ 
		\hline
		\cellcolor{gray!10}
		\textbf{Conflict: Dependency Conflicts} \newline
		The Jetson Nano relies on JetPack 4.6 (Ubuntu 18.04), which locks the environment to \textbf{Python 3.6}.
		\newline \newline
		$\bullet$ \textbf{Incompatibility:} State-of-the-art libraries (Ultralytics YOLOv8, Pandas 2.0) require Python 3.8+ and modern C++ compilers, causing installation failures. \newline
		$\bullet$ \textbf{Deprecated APIs:} The \texttt{Pillow} image library recently removed attributes (like \texttt{ANTIALIAS}) used by older Jetson inference tools, causing runtime crashes.
		& 
		\cellcolor{gray!10}
		\textbf{Solution: Cross-Compilation Workflow} \newline
		We bypassed the OS limitations using an external pipeline:
		\newline \newline
		$\bullet$ \textbf{TensorRT Export:} Instead of running PyTorch natively, we trained the model on a PC, exported it to \textbf{ONNX}, and cross-compiled it into a \textbf{.engine} binary specifically optimized for the Nano's Maxwell GPU. \newline
		$\bullet$ \textbf{Monkey Patching:} We developed a custom initialization script that injects code at runtime to manually map old API calls to their new locations, preventing crashes in legacy libraries. \\
		\hline
	\end{tabular}
}

% ==============================================================================
% SLIDE 3: REMOTE DEV CHALLENGE
% ==============================================================================
\STANDARD{Challenge 4: Remote Development Breakdown}
{
	\renewcommand{\arraystretch}{1.5}
	\small
	
	\noindent
	\begin{tabular}{|p{0.48\linewidth}|p{0.48\linewidth}|}
		\hline
		\cellcolor{red!40!black}\textbf{\color{white} The Problem} & 
		\cellcolor{blue!40!black}\textbf{\color{white} The Engineered Solution} \\ 
		\hline
		\cellcolor{gray!10}
		\textbf{Conflict: glibc Version Lock} \newline
		In 2024, Microsoft updated the VS Code Remote Server to require \textbf{glibc 2.28} or higher.
		\newline \newline
		$\bullet$ \textbf{The Lockout:} The Jetson Nano's OS is pinned to \textbf{glibc 2.27}. \newline
		$\bullet$ \textbf{Impact:} Standard SSH connections failed with OS Not Supported errors, blocking all direct code editing and file transfer capabilities on the device.
		& 
		\cellcolor{gray!10}
		\textbf{Solution: SSH-FS Protocol Switch} \newline
		We architected a workaround that avoids running binary agents on the Jetson:
		\newline \newline
		$\bullet$ \textbf{SFTP Mounting:} We switched to the \textbf{SSH FS} extension, which mounts the Jetson's external SSD (/media/myssd) as a local network drive on the development laptop. \newline
		$\bullet$ \textbf{Benefit:} This protocol relies purely on standard SFTP, which does not check the glibc version, allowing full real-time coding access despite the OS obsolescence. \\
		\hline
	\end{tabular}
}

% ==============================================================================
% SLIDE 4: LOGIC CHALLENGE
% ==============================================================================

\Mysection{System Setup}		
\STANDARD{Setup with SSH connection}
{
	\begin{itemize}
		\item \textbf{Unit:} NVIDIA Jetson Nano.
		\item \textbf{Camera:} Microsoft LifeCam for video input.
		\item \textbf{Control:} Laptop via SSH for debugging.
		\item \textbf{UI:} External monitor showing Flask Dashboard.
	\end{itemize}
	
	\begin{center}
		% Reduced width from 0.75 to 0.6 to fit on slide
		\includegraphics[width=0.6\textwidth]{img/setup.jpg}
	\end{center}
	
	\centering
	\textit{\small The hardware setup}
}
\Mysection{Results}
\STANDARD{System Demonstration Video}
{
	\centering
	% This command simply opens the file "demo.mp4" in your default video player
	\href{run:./liveDemo.mp4}{\includegraphics[width=0.8\textwidth]{img/setup.jpg}}
	
	\vspace{0.3cm}
	\textit{\small Click image to launch video player.}
}
\Mysection{Results}

\STANDARD{Model Performance}
{
	\begin{itemize}
		\item \textbf{YOLO Detection:} 100\% accurate. The model "locks on" to the license plate perfectly in every frame, even with movement.
		\item \textbf{OCR Limitations:} Text recognition is inconsistent. While detection is perfect, the OCR sometimes misinterprets characters (e.g., swapping order due to angle).
		\item \textbf{Latency:} The video feed pauses briefly during recognition, confirming that the heavy OCR processing is happening locally on the Jetson Nano.
	\end{itemize}
}

\STANDARD{System Logic Verification}
{
	\begin{itemize}
		\item \textbf{Handling Misreads:} When the OCR reads a wrong number (e.g., \texttt{66ZLA}), the database logic correctly treats it as a New Vehicle entry.
		\item \textbf{Successful Billing:} Despite the OCR challenge, when the text \textit{is} read correctly, the billing math and timestamp updates work perfectly (\textbf{\euro 99.20}).
	\end{itemize}
}