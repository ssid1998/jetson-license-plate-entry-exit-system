...
if __name__ == '__main__':
    ...
    img_names_undistort = []
    print('img: ', img_names)
    #for loop to read image names and the images from the folder
    for fn in img_names:
        ...
        #the above function finds all the intersection points on the chess board image 
        if found:
            term = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_COUNT, 30, 0.1)
            #cv2.TERM_CRITERIA_EPS - stop the algorithm iteration if specified accuracy, epsilon, is reached
            cv2.cornerSubPix(img, corners, (5, 5), (-1, -1), term)
            #cv2.cornerSubPix(image, corners, winSize, zeroZone, criteria)
            #This function uses the dot product trick and iteratively refines the corner locations till the termination criteria is reaches
        if out:
            vis = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
            #cv2.cvtColor() method is used to convert an image from one color space to another. 
            cv2.drawChessboardCorners(vis, pattern_size, corners, found)
            #connects all the intersection points on the chessboard
