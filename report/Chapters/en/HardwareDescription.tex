...
\begin{figure}[H]
	...
	\caption{Jetson Nano Ports \cite{NVU:2025}}
	\label{fig:camera_module}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Jetson Nano Pins \cite{NVU:2025}}
	\label{fig:camera_module}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Jetson Nano Rear View \cite{NVU:2025}} 
	\label{fig:camera_module}
\end{figure}
...
\begin{itemize}
	...
	\item Carrier Board
	\begin{itemize}
		\item (DS1) Power LED which lights up when the developer kit is powered on.
		\item (J1) SO-DIMM connector for attaching the Jetson Nano module.
		\item (J6) 40-pin Expansion Header for GPIO and other peripheral interfaces.
		\item (J5) Camera connector supports CSI cameras, compatible with IMX219 camera modules, such as the Leopard Imaging LI-IMX219-MIPIFF-NANO and Raspberry Pi Camera Module V2.
		\item (J7) 4-pin Fan Header supports PWM (Pulse Width Modulation) output and tachometer input.
		\item (J10) USB 2.0 Type-A (x2) for connecting USB 2.0 devices.
		\item (J9) USB 3.0 Type-A High-speed USB interface for data transfer.
		\item (J4) HDMI Type-A Video output for monitors or displays.
		\item (J2) USB Type-C Power Connector Supplies power to the board.
		\item (J3) Ethernet Jack for network connectivity.
		\item (J12) Button Header exposes system signals for power, reset, and force recovery.
		\item (J13) USB Micro-B: Can function for powering the kit (5V 2A) or operate in device mode if J48 pins are connected.
	\end{itemize}
\end{itemize}
...
\section{Hardware List and Bill of Materials}
The following List and Bill of Materials has been identified for the Hardware:
\begin{table}[h!]
	\begin{tabular}{||m{1cm}|m{2.2cm}|m{2.5cm}|m{1.5cm}|m{1.7cm}|m{1cm}|m{3cm}||}
		...
		\hline
		1 & NVIDIA Jetson Nano Developer Kit & 128-core Maxwell GPU, a quad-core ARM Cortex-A57 CPU, 4GB LPDDR4 memory & \includegraphics[width=1.5cm]{Images/Developer_Kit} & 1 & 214€ & \href{https://www.seeedstudio.com/Jetson-10-1-A0-p-5336.html}{Jetson-Nano}\\
		\hline
		...
		\hline
		1 & Raspberry Pi Power Supply &  5V/3A Type-C power adapter & \includegraphics[width=1cm]{Images/Raspberry_Pi} & 1 & 14.95€ & \href{https://www.ebay.de/itm/405022626215?_skw=5v+3a+usb+power+supply&itmmeta=01JH89C62A7R2XR9N6YWC1XE4R&hash=item5e4d3ad1a7:g:3rcAAOSw8rVmYs~3&itmprp=enc%3AAQAJAAAA8HoV3kP08IDx%2BKZ9MfhVJKkHwtiaZVoAnpD43wu013E6I5Q9pd3AsIdC11mREAx%2B8Bc3Jse9Aj8YxTqQv%2BGFPERv7n%2B4p3FwGlQDzu5LI9GPFML%2BWQXLfHEa9DH%2F8Tes1buscg%2Bd9HQaybJp7kM3T8IDhnx44%2BgPeVDA%2FFkBLF3faiYjlRdndWnWY%2F0WI%2FmHIJGNgzIqeKL5KSWWHqVgZMlp7ZWfCHwbXoDlC%2FP2Rxk0HqnL0SME9YuayjmpVMeP7I7sUeugdWFWBlLRYiHco0DxHKshXResydt6blgzqGquNytbmgPiYPGMBHceEVJcOQ%3D%3D%7Ctkp%3ABFBMtOGwiYpl}{Raspberry Pi}\\
		\hline
		...
		\hline
		1 & Keyboard & Standard American & \includegraphics[width=1.5cm]{Images/Keyboard} & 1 & 23€ & \href{https://www.reichelt.de/funk-tastatur-usb-schwarz-delock-12004-p293085.html?&trstct=pol_0&nbc=1}{Keyboard}\\
		\hline
...
\subsection{Description of Materials}

\begin{itemize}[left=0pt]
	\item \textbf{NVIDIA Jetson Nano Developer Kit}: The Jetson Nano, equipped with a 128-core Maxwell GPU and a quad-core ARM Cortex-A57 CPU, is the brain of the car license plate identification system. With 4GB of LPDDR4 memory, it is capable of running complex machine learning models for real-time image and video processing. The Jetson Nano is ideal for tasks like optical character recognition (OCR) and computer vision, which are essential for detecting and identifying license plates from car images or videos captured by the camera. \cite{SED:2025}
	
	\item \textbf{Logitech C270 HD Webcam}: The Logitech C270 HD webcam is used to capture high-definition video footage of vehicles for license plate detection. With its 1280 x 720 resolution at 30fps, this webcam provides clear video, which is crucial for accurately capturing license plate details. The webcam’s plug-and-play functionality makes it easy to integrate with the Jetson Nano, and its 3 MP camera ensures that fine details like the text on the license plate are captured with clarity. \cite{CLB:2025}
	
	\item \textbf{Micro SD Card (Minimum 32GB UHS-1)}: The 32GB UHS-1 microSD card is used for storing the Jetson Nano’s operating system, machine learning models, and captured data. With high read/write speeds, this card ensures fast access to the system and quick data processing, which is vital when processing video footage of cars in real time for license plate recognition. \cite{ECK:2025}
	
	\item \textbf{Raspberry Pi Power Supply (5V/3A Type-C)}: The 5V/3A Type-C power supply is essential for providing stable and sufficient power to the Jetson Nano. The power supply ensures that the Jetson Nano can handle the processing demands of the license plate identification software, such as running deep learning models and managing the high throughput of video data from the webcam. \cite{eBay:2025}
	
	\item \textbf{HDMI Cable}: The HDMI cable is used to connect the Jetson Nano to an external monitor for visual output during development and testing. It allows you to view live video feeds and monitor the results of the license plate identification system, providing real-time feedback on the system's accuracy and performance. \cite{AMZ:2025}
	
	\item \textbf{Keyboard}: The keyboard is used for system configuration and interacting with the Jetson Nano during setup and testing. It enables input for coding, running scripts, and configuring the license plate recognition system, ensuring that any necessary adjustments or debugging steps can be performed efficiently. \cite{RCT:2025}
	
	\item \textbf{Mouse (Standard 3-Button Wired or Wireless)}: The mouse is used to interact with the system’s graphical user interface (GUI) for configuration and monitoring. It is essential for selecting options, navigating through the application, and checking results during the operation of the car license plate identification system. \cite{RMS:2025}
\end{itemize}
