...
\section*{Linux Operating System on Jetson Nano}

The NVIDIA Jetson Nano runs on a Linux-based operating system called \textbf{Jetson Linux} (formerly known as Linux for Tegra or L4T). This is a customized version of Ubuntu tailored specifically for the Jetson family of devices. The operating system is optimized to leverage the unique hardware capabilities of the Jetson Nano, such as the GPU for accelerated computing, making it an ideal platform for edge AI and embedded applications like license plate recognition. \cite{NSM:2025}

\subsection{Key Features of Jetson Linux on Jetson Nano}

\begin{itemize}
	\item \textbf{Base OS:} Jetson Linux is based on Ubuntu (currently Ubuntu 18.04 or 20.04 for newer versions). It provides a familiar Linux environment for developers.
	\item \textbf{Kernel Customization:} Jetson Linux includes a custom Linux kernel designed to support NVIDIA GPUs, enabling hardware acceleration for applications requiring high computational power.
	\item \textbf{Package Management:} The system uses the standard \texttt{apt} package manager, allowing users to install and update software easily.
	\item \textbf{NVIDIA JetPack SDK:}
	\begin{itemize}
		\item JetPack is a comprehensive software development kit for the Jetson platform. It includes libraries, APIs, and tools for AI, computer vision, and deep learning applications.
		\item Key components include TensorRT, cuDNN, CUDA, and multimedia libraries optimized for NVIDIA GPUs.
	\end{itemize}
\end{itemize}

\subsection{Advantages of Using Linux for Jetson Nano}

\begin{itemize}
	\item \textbf{Open-Source Flexibility:} Linux provides access to a vast array of open-source libraries and tools.
	\item \textbf{Hardware Optimization:} Jetson Linux is optimized to utilize the Jetson Nano's GPU (NVIDIA Maxwell architecture with 128 CUDA cores) for parallel processing tasks.
	\item \textbf{Ease of Development:} With a user-friendly GUI (Ubuntu desktop environment), developers can easily code, debug, and deploy applications.
	\item \textbf{Long-Term Support:} NVIDIA regularly updates Jetson Linux to support newer Jetson devices and maintain compatibility with AI/ML frameworks.
\end{itemize}

\subsection{Typical Workflow with Jetson Linux}

\begin{enumerate}
	\item \textbf{Flashing the OS:}
	\begin{itemize}
		\item The Jetson Nano's SD card is flashed with the Jetson Linux image using tools like Balena Etcher.
		\item Official images can be downloaded from NVIDIA's Jetson Download Center
	\end{itemize}
	\item \textbf{Initial Setup:}
	\begin{itemize}
		\item After flashing, users configure the OS by setting up the username, password, and network.
	\end{itemize}
	\item \textbf{Installing Development Tools:}
	\begin{itemize}
		\item NVIDIA JetPack SDK is installed via the SDK Manager or using pre-installed tools in the Jetson Linux image.

	\end{itemize}
	\item \textbf{Application Development:}
	\begin{itemize}
		\item Applications can be developed using Python with libraries like OpenCV, TensorFlow, or PyTorch.
		\item Optimized AI models can be deployed using NVIDIA TensorRT for inference.
	\end{itemize}
\end{enumerate}

\subsection{Applications of Jetson Linux}

\begin{itemize}
	\item \textbf{Edge AI:} Run AI inference tasks locally on the device without depending on cloud resources.
	\item \textbf{IoT Devices:} Integrate with sensors and other hardware for industrial IoT applications.
	\item \textbf{Computer Vision:} Perform tasks such as object detection, facial recognition, and license plate identification.
\end{itemize}
