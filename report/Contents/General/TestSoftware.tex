...
Following the validation of the hardware components, the next critical phase involves verifying the software ecosystem. The project relies on a specific stack of software tools, including the NVIDIA JetPack OS, Python development environment, and key machine learning libraries such as OpenCV, PyTorch, and YOLOv8. This chapter details the procedures used to verify the correct installation and functionality of each software module on the Jetson Nano.

\section{Software Stack Overview}
The software architecture is built upon the NVIDIA JetPack SDK, which provides the Linux operating system (L4T) and essential acceleration libraries (CUDA, cuDNN, TensorRT). The application layer is developed in Python, leveraging specific libraries for computer vision and data management.

\begin{table}[h]
	\centering
	\caption{Software Bill of Materials and Verification Targets \cite{Stu24, Ope24, PyT25}}
	\label{tab:sw_bom}
...
The software verification process confirmed the stability and interoperability of the development stack. The JetPack OS correctly exposes the GPU to the Python environment, allowing PyTorch and YOLOv8 to perform accelerated inference. OpenCV successfully handles image operations, and SQLite ensures reliable data logging. These tests validate the software readiness for the main application deployment.