...
\chapter{\textcolor{red}{Development Environment -- Python}}
...
\noindent
	...
	\begin{itemize}
		...
		\item \textbf{Detection runtime:} TensorRT engine (\FILE{/media/myssd/jetson-inference/data/networks/best.engine}) built from a YOLOv8 model exported to ONNX.
		\item \textbf{OCR runtime:} EasyOCR (\texttt{easyocr==1.7.1}), configured to run on CPU (\PYTHON{gpu=False}) to reduce GPU memory pressure.
		\item \textbf{UI layer:} Flask web dashboard (live view + billing table) served from the Jetson (\TBD{confirm final port and launch command}).
	\end{itemize}
...
\begin{itemize}
		...
		\item \textbf{Post-processing:} \TBD{confirm whether IoU-based NMS is implemented in the final demo script, or if confidence filtering/top-1 selection is used.}
\end{itemize}
...
\chapter{\textcolor{red}{Development Environment -- Hardware / OS / System Tools}}
...
\begin{itemize}
	...
	\item \textbf{TensorRT engine build (on-device):} \TBD{confirm final command and ONNX source path; example: \SHELL{/usr/src/tensorrt/bin/trtexec --onnx=best.onnx --saveEngine=best.engine --fp16}.}
	\end{itemize}
