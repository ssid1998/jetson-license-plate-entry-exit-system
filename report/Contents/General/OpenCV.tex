...
\begin{figure}[htbp]
	\centering
	\input{../../tikz/4DomainKnowledgeML/OpenCV/OpenCVPipeline.tikz}
	\caption{OpenCV's role in the real-time licence plate detection workflow: camera I/O, pre-processing, overlays, and visual debugging around the model inference stage.}
	\label{fig:opencv_pipeline}
\end{figure}
...
For this project, OpenCV is used via its Python bindings (\PYTHON{cv2}) in the JetPack~4.6.x environment (L4T R32.7.1) on the Jetson Nano B01. There are two common installation strategies:
...
%	\end{verbatim}
	...
	\item \textbf{Step 5:} A window opens displaying the camera feed consisting of a grayscale conversion (requires a connected monitor / GUI session). 
